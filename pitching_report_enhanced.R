...
create_movement_plot <- function(data) {
  # Enhanced movement plot with arm angle information
  avg_movement <- data %>%
    group_by(PitchType) %>%
    summarise(
...
create_release_plot <- function(data) {
  ...
  if ("ArmAngle" %in% names(data) && any(!is.na(data$ArmAngle))) {
    ...
    release_plot <- release_plot +
      geom_segment(data = arm_angle_lines, 
                   aes(x = x_start, y = y_start, xend = x_end, yend = y_end, color = PitchType),
...
create_tilt_plot <- function(data) {
  ...
  if ("SpinAxis" %in% names(data)) {
    ...
    # Calculate average arm angle for each pitch type and create lines
  ...
  tilt_plot <- ggplot() +
    ...
    theme_void() +
    labs(title = "Tilt Consistency with Arm Angles") +
...
create_heatmaps_by_pitch <- function(data) {
  ...
  # Debug: Print pitch type counts
  pitch_counts <- data_clean %>% count(PitchType)
  print("Pitch type counts for heatmaps:")
  print(pitch_counts)
  ...
  # Debug: Check variance in location data
  location_variance <- data_clean %>%
    group_by(PitchType) %>%
    summarise(
      var_x = var(PlateLocSide, na.rm = TRUE),
      var_y = var(PlateLocHeight, na.rm = TRUE),
      n = n(),
      .groups = 'drop'
    )
  print("Location variance by pitch type:")
...
# ORIGINAL SINGLE-PAGE FUNCTION (ENHANCED WITH ARM ANGLES)
